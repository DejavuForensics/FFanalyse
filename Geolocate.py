...
class Geolocate:
    ...
    def listCities(self,ip):
        location = self.getLocation(ip) #get the location from ip
        ...
        for i in ips:
            ...
            if location: #Location exists in MaxMind
                ...
                if tt == '' or tt== None: #timezone not available, set to GMT as default
                    tt='GMT' 
                tz = timezone(tt)
                locd = tz.localize(datetime(2012, 1, 1, 0, 0, 0))
                ...
                timezones.append(d)
                utms.append(uv) 
                print('Universal Transverse Mercator (UTM): ' + str(uv))
                print('Military Grid Reference System (MGRS): ' + str(mgr[0]))
